...
std::tuple<pylir::ARM64::Adjustments::Arg, mlir::Type, mlir::Type>
    pylir::ARM64::flattenSingleArg(mlir::Type type, std::uint8_t* takenIntegers, std::uint8_t* takenFloats)
{
...
mlir::LLVM::LLVMFuncOp pylir::ARM64::declareFunc(mlir::OpBuilder& builder, mlir::Location loc, mlir::Type returnType,
...
mlir::Value pylir::ARM64::callFunc(mlir::OpBuilder& builder, mlir::Location loc, mlir::LLVM::LLVMFuncOp func,
                                    mlir::ValueRange operands)
{
    ...
    for (auto iter = operands.begin(); iter != operands.end(); iter++)
    {
        auto getPointerToMemory = [&](mlir::Value value) -> mlir::Value
        {
            ...
            auto one = builder.create<mlir::LLVM::ConstantOp>(loc, builder.getI32Type(), builder.getI32IntegerAttr(1));
            auto tempAlloca =
                builder.create<mlir::LLVM::AllocaOp>(loc, mlir::LLVM::LLVMPointerType::get((*iter).getType()), one, 1);
...
pylir::ARM64::ARM64(mlir::DataLayout dataLayout) : PlatformABI(std::move(dataLayout)) {}
